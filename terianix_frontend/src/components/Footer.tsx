    ...
    <footer className="mt-auto bg-[#0f0d18] text-white/70">
      <div className="mx-auto max-w-7xl px-6 py-14 lg:px-10">
        ...
        <div className="mt-12 flex flex-col items-start justify-between gap-3 border-t border-white/10 pt-6 text-xs text-white/50 sm:flex-row sm:items-center">
          <span>&copy; {new Date().getFullYear()} Terianix by Terian Services. All rights reserved.</span>
          <a href="mailto:support@terian-services.com" className="text-white/60 hover:text-violet-300">
  ...
  return (
    <div>
      ...
      <ul className="mt-4 space-y-2 text-sm">
        {items.map((item) => (
          <li key={item.href}>
            <a href={item.href} className="text-white/75 transition hover:text-violet-300">
              {item.label}
            </a>
          </li>
        ))}
