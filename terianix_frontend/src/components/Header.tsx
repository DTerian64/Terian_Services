  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#0f0d18] text-white">
      <div className="mx-auto flex h-[5.4rem] max-w-7xl items-center justify-between px-6 lg:px-10">
        <a href="/" className="flex items-center gap-4 text-white no-underline" aria-label="Terianix home">
          <img src="/logo.svg" alt="" className="-my-1 h-[3.6rem] w-auto object-contain" />
          <span className="font-playfair text-2xl font-bold tracking-normal">Terianix.ai</span>
        </a>
        ...
        <nav className="hidden items-center gap-10 text-[15px] font-semibold text-white lg:flex" aria-label="Primary navigation">
          ...
          {NAV_ITEMS.map((item) => (
            <a key={item.href} href={item.href} className="py-3 text-white transition hover:text-violet-300">
              {item.label}
            </a>
          ))}
          ...
          <a
            ...
            className="inline-flex items-center gap-2 rounded-md px-1 py-3 text-[15px] font-semibold text-white transition hover:text-violet-300"
          >
            <SparkleIcon />
            {ASK_AI_ITEM.label}
          </a>
        ...
        <button
          ...
          className="inline-flex h-11 w-11 items-center justify-center rounded-md border border-white/15 text-white transition hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-300 lg:hidden"
          ...
          <div className="mx-auto max-w-7xl space-y-1">
            ...
            {NAV_ITEMS.map((item) => (
              <a
                ...
                className="block rounded-md px-3 py-3 text-base font-semibold text-white transition hover:bg-white/10 hover:text-violet-300"
              >
                {item.label}
              </a>
            ))}
            ...
            <a
              href={ASK_AI_ITEM.href}
              onClick={() => setMenuOpen(false)}
              className="mt-2 flex items-center gap-2 rounded-md px-3 py-3 text-base font-semibold text-white transition hover:bg-white/10 hover:text-violet-300"
            >
              <SparkleIcon />
              {ASK_AI_ITEM.label}
            </a>
  ...
  return (
    <div className="relative" onMouseEnter={showMenu} onMouseLeave={() => setOpen(false)}>
      {href ? (
        <a
          href={href}
          className="flex items-center gap-2 rounded-md px-1 py-3 font-semibold text-white transition hover:text-violet-300"
        >
          {label}
        ...
        <button
          type="button"
          className="flex items-center gap-2 rounded-md px-1 py-3 text-white transition hover:text-violet-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-300"
          aria-haspopup="menu"
          aria-expanded={open}
        ...
        <div className={`absolute left-0 top-full ${widthClass} rounded-md border border-white/10 bg-[#151321] p-2 shadow-2xl shadow-black/30`} role="menu">
          {items.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="flex items-center justify-between gap-3 rounded px-4 py-3 text-sm text-white/90 transition hover:bg-white/10 hover:text-violet-300"
              role="menuitem"
            >
    ...
    <div>
      ...
      {items.map((item) => (
        <a
          ...
          href={item.href}
          onClick={onNavigate}
          className="flex items-center justify-between rounded-md px-3 py-3 text-base font-semibold text-white transition hover:bg-white/10 hover:text-violet-300"
        >
          <span>{item.label}</span>
          {item.badge ? (
            <span className="rounded-full bg-violet-500/15 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-violet-300">
...
function SparkleIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5 text-violet-400" fill="currentColor">
