...
function Avatar({ name, photo_url }: { name: string; photo_url: string }) {
  ...
  return (
    <div className="flex h-20 w-20 items-center justify-center rounded-full bg-violet-500/20 ring-2 ring-violet-400/30">
      <span className="text-xl font-bold text-violet-300">{initials(name)}</span>
    </div>
  );
}
...
function MemberCard({ member }: { member: TeamMember }) {
  return (
    <div className="rounded-xl border-2 border-white/10 bg-[#0f0d18] p-6 transition hover:border-violet-400 flex gap-6 items-start">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm font-medium text-violet-400">{member.title}</p>
        ...
        {member.linkedin_url && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-violet-400 hover:text-violet-300 transition"
          >
            LinkedIn →
          </a>
        )}
