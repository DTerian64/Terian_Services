  ...
  return (
    <div
      className={`relative flex flex-col rounded-2xl p-8 transition-all duration-200 ${
        tier.highlight
          ? "border-2 border-violet-400 bg-[#0f1a19]"
          : "border border-white/10 bg-[#0f0d18] hover:border-white/25"
      }`}
    >
      {tier.highlight && (
        <span className="absolute -top-3.5 left-8 rounded-full bg-violet-500 px-3 py-0.5 text-[11px] font-bold uppercase tracking-widest text-[#0f0d18]">
          Most popular
        </span>
      )}
      ...
      <a
        ...
        className={`mt-8 flex w-full items-center justify-center rounded-lg px-5 py-3 text-sm font-bold uppercase tracking-wider transition ${
          tier.highlight
            ? "bg-violet-500 text-[#0f0d18] hover:bg-violet-400"
        ...
        {doc && (
          <>
            ...
            <div className="mt-8 flex items-center gap-3">
              ...
              <span className={`text-sm ${annual ? "text-slate-100" : "text-slate-500"}`}>
                Annual
                <span className="ml-2 rounded-full bg-violet-500/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-violet-300">
                  Save ~20%
                </span>
              </span>
            </div>
