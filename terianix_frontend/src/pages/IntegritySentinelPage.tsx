    ...
    <PageLayout>
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-6 py-16 lg:grid-cols-[1fr_0.95fr] lg:px-10 lg:py-24">
          <div className="flex flex-col justify-start">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <a
                href={CONTACT_HREF}
                className="inline-flex items-center justify-center rounded-md bg-violet-500 px-6 py-3 text-sm font-bold uppercase tracking-wider text-slate-100 transition hover:bg-violet-400"
              >
        ...
        <div className="mx-auto max-w-6xl px-6 py-20 lg:px-10">
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {architecture.map((item) => (
              <div key={item.layer} className="rounded-lg border-2 border-white/10 bg-[#0a0916] transition hover:border-violet-400 p-5">
      ...
      <section className="bg-[#0f0d18] text-white">
        <div className="mx-auto grid max-w-6xl gap-8 px-6 py-16 lg:grid-cols-[1fr_auto] lg:items-center lg:px-10">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-300">Product status</p>
        ...
        <div className="grid gap-5 p-5 md:grid-cols-[1fr_0.9fr]">
          <div className="min-h-[22rem] rounded-md border border-white/10 bg-[#0f0d18] p-5">
            <div className="relative h-full min-h-[19rem]">
              <GraphNode className="left-[8%] top-[8%]" label="Employee" tone="violet" />
...
function GraphNode({
  ...
  label: string;
  tone: "violet" | "amber" | "rose" | "slate" | "indigo";
  className: string;
  large?: boolean;
}) {
  const toneClass = {
...
function ConsolePanel({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-md border border-white/10 bg-white/[0.04] p-4">
      <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-violet-300">{title}</p>
